...
const App: React.FC = () => {
  ...
  // Đọc danh sách sản phẩm từ file public/products.xlsx ngầm
  useEffect(() => {
  ...
  // Tự động đọc file note thông báo (notices.txt) khi vào web
  useEffect(() => {
    const loadNoticesFromTxt = async () => {
      ...
      } catch (err) { console.error("Lỗi ngầm khi đọc file note thông báo:", err); }
    };
    loadNoticesFromTxt();
  }, []);
  ...
  return (
    <Layout theme={theme} toggleTheme={toggleTheme}>
      ...
      {/* ====================================================================
          VỊ TRÍ MỚI NÂNG CẤP: PHÁT SÓNG THÔNG BÁO TỐI ƯU FIXED TRÊN GÓC PHẢI MÀN HÌNH
          (Không nằm cạnh hòm file, giao diện mở rộng to ra không lo bị che chữ)
          ==================================================================== */}
      {notices.length > 0 && (
        <div className="fixed top-24 right-4 lg:right-8 z-40 hidden 2xl:block animate-slide-up" style={{ animationDelay: '0.3s' }}>
          <div className={`p-6 rounded-3xl border-2 transition-all duration-500 shadow-2xl w-[320px] space-y-4 ${
            ...
            <div className="space-y-3.5 max-h-[360px] overflow-y-auto pr-1 custom-scrollbar">
              {notices.map(notice => (
                <div 
                  key={notice.id} 
                  className={`p-3 rounded-xl border flex gap-3 hover:scale-[1.03] transition-transform ${
                ...
                >
                  <div className={`w-12 h-12 flex-shrink-0 rounded-xl flex flex-col items-center justify-center font-mono font-black text-xs border ${
                    isOcean ? 'bg-cyan-950/80 border-cyan-500/40 text-cyan-300 shadow-[0_0_10px_rgba(34,211,238,0.15)]' : 'bg-gradient-to-br from-red-500 to-red-600 border-red-400 text-white'
                  }`}>
                    {notice.date}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className={`text-xs font-black truncate ${isOcean ? 'text-cyan-100' : 'text-red-950'}`}>{notice.title}</p>
                    <p className="text-[11px] opacity-85 leading-normal mt-1 text-justify pr-1">{notice.desc}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* KHU VỰC CHỨA CẤU TRÚC ĐƠN HÀNG (QUAY LẠI GRID 2 CỘT CÂN ĐỐI) */}
      <div className="flex flex-col gap-10 relative z-10">
        ...
        {/* BỐ CỤC CHÍNH QUAY VỀ 2 CỘT CÂN ĐỐI (4 CỘT LỊCH SỬ - 8 CỘT KHUNG CHÍNH) */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start px-4 max-w-6xl mx-auto w-full">
